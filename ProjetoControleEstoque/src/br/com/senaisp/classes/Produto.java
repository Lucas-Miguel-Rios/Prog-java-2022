...
public class Produto {
	...
	public boolean update() {
		...
		try {
			...
			//desconctando do banco de dados 
			conn.desconectarBD();
			ret = true;
		} catch (SQLException e) {
			haErro =true;
	...
	public boolean delete() {
		...
		try {
			...
			PreparedStatement stmt = conn.getConector().prepareStatement(sql);
			//Setando parametro para executar 
			stmt.setInt(1, id);
			//Executando a instrução
			int rowsAff = stmt.executeUpdate();
			ret = true;
			//log
			System.out.println("Registro deletados: " +rowsAff);
			conn.desconectarBD();
		} catch (SQLException e) {
			haErro = true;
			msgErro = e.getMessage();
	...
	public DefaultTableModel getListaProdutos() {
		...
		String sql = "select id, descricao, saldo, preco, local_estoque from produtos order by id limit 100"; //limit 100 é limitação de registro que virá do banco de dados 
		try {
			conn.conectarBD();
			//preparando a instrução
			PreparedStatement stmt = conn.getConector().prepareStatement(sql);
			...
			ResultSet rs = stmt.executeQuery();
			//Obtendo a instrução dos registros que retornaram 
