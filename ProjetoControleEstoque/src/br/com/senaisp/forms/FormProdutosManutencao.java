		...
		public void actionPerformed(ActionEvent e) {
			...
			if (obj.getText() == "Confirmar") {
				...
				if (tipoOperacao != 4) {
					...
					if (JOptionPane.showConfirmDialog(null, "Confirmar Exclusão?", "Confirmação de Exclusão",
							JOptionPane.OK_CANCEL_OPTION) == JOptionPane.OK_OPTION) {
						bolTeste = prod.delete();
					}
				}
				if (!bolTeste) {
					JOptionPane.showMessageDialog(null, "Ocorreu um problema! Erro:" + prod.getMsgErro());
				}
				dispose();
